...
func (s *SmartContract) Mint(ctx contractapi.TransactionContextInterface, amount float64) error {
	...
	if currentBalanceBytes == nil {
		...
		currentBalance, err = strconv.ParseFloat(string(currentBalanceBytes), 64) // Error handling not needed since Itoa() was used when setting the account balance, guaranteeing it was an integer.

      if err == nil{
         return errors.New("currentBalance Float64 Convert Error")
      } // Error handling not needed since Itoa() was used when setting the account balance, guaranteeing it was an integer.
	}
	...
	if totalSupplyBytes == nil {
		...
		if err == nil {
			return errors.New("totalSupply error Float64")
		}
	}
...
// This function triggers a Transfer event
func (s *SmartContract) Burn(ctx contractapi.TransactionContextInterface, amount float64) error {
...
func (s *SmartContract) BalanceOf(ctx contractapi.TransactionContextInterface, account string) (float64, error) {
	...
	balance, _ := strconv.ParseFloat(string(balanceBytes), 64) // Error handling not needed since Itoa() was used when setting the account balance, guaranteeing it was an integer.
...
func (s *SmartContract) TotalSupply(ctx contractapi.TransactionContextInterface) (float64, error) {
...
func (s *SmartContract) Approve(ctx contractapi.TransactionContextInterface, spender string, value float64) error {
	...
	err = ctx.GetStub().PutState(allowanceKey, []byte(strconv.FormatFloat(value, 'f', -1, 64)))
	if err != nil {
...
func (s *SmartContract) Allowance(ctx contractapi.TransactionContextInterface, owner string, spender string) (float64, error) {
	...
	if allowanceBytes == nil {
		...
		allowance, err = strconv.ParseFloat(string(allowanceBytes), 64) // Error handling not needed since Itoa() was used when setting the totalSupply, guaranteeing it was an integer.
	}
...
// This function triggers a Transfer event
func (s *SmartContract) TransferFrom(ctx contractapi.TransactionContextInterface, from string, to string, value float64) error {
	...
	var currentAllowance float64
	currentAllowance, _ = strconv.ParseFloat(string(currentAllowanceBytes), 64) // Error handling not needed since Itoa() was used when setting the totalSupply, guaranteeing it was an integer.
	...
	err = ctx.GetStub().PutState(allowanceKey, []byte(strconv.FormatFloat(updatedAllowance, 'f', -1, 64)))
	if err != nil {
		return err
	}
...
func transferHelper(ctx contractapi.TransactionContextInterface, from string, to string, value float64) error {
	...
	fromCurrentBalance, _ := strconv.ParseFloat(string(fromCurrentBalanceBytes),64) // Error handling not needed since Itoa() was used when setting the account balance, guaranteeing it was an integer.
	...
	var toCurrentBalance float64
	// If recipient current balance doesn't yet exist, we'll create it with a current balance of 0
	if toCurrentBalanceBytes == nil {
		toCurrentBalance = 0
	} else {
		toCurrentBalance, _ = strconv.ParseFloat(string(toCurrentBalanceBytes), 64) // Error handling not needed since Itoa() was used when setting the account balance, guaranteeing it was an integer.
	}

	fromUpdatedBalance := fromCurrentBalance - value
	toUpdatedBalance := toCurrentBalance + value

	err = ctx.GetStub().PutState(from, []byte(strconv.FormatFloat(fromUpdatedBalance, 'f', -1, 64)))
	if err != nil {
		return err
	}

	err = ctx.GetStub().PutState(to, []byte(strconv.FormatFloat(toUpdatedBalance, 'f', -1, 64)))
	if err != nil {
		return err
	}
